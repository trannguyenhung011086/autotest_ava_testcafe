...
export const AccountForgotTest = () => {
    it('POST / empty email', async () => {
        let res = await request.post(config.api.forgot, { "email": "" })
    ...
    it('POST / non-existing email', async () => {
        let res = await request.post(config.api.forgot, { "email": 'QA_' + faker.internet.email() })
        expect(res.statusCode).toEqual(404)
        expect(res.body.message).toEqual('EMAIL_NOT_EXIST')
    })

    it('POST / wrong format email', async () => {
        let res = await request.post(config.api.forgot, { "email": ".test%!@#$%^&*()_+<>?@mail.com" })
        expect(res.statusCode).toEqual(400)
        expect(res.body.message).toEqual('REGISTER_INVALID_EMAIL')
    })

    it('POST / Facebook email', async () => {
        let res = await request.post(config.api.forgot, { "email": "trannguyenhung011086@gmail.com" })
        expect(res.statusCode).toEqual(404)
        expect(res.body.message).toEqual('COULD_NOT_RESET_OF_FACEBOOK_ACCOUNT')
    })

    it('POST / existing email', async () => {
        let res = await request.post(config.api.forgot, { "email": config.testAccount.email })
