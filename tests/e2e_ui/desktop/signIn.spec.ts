...
test("Cannot sign in with empty email and password", async () => {
	...
	await t
		.expect(await page.signIn.getEmailError())
		.eql(config.notifyMsg.missingEmail)
		.expect(await page.signIn.getPasswordError())
		.eql(config.notifyMsg.missingPassword);
});

test("Cannot sign in with non-existing email", async () => {
	await page.signIn.submitData(
		faker.internet.email(),
		faker.internet.password()
	);
	await t
		.expect(await page.signIn.getCommonError())
		.eql(config.notifyMsg.invalidEmailPassword);
});
...
test("Can sign in with existing email", async () => {
	...
	await t
		.expect(await page.signIn.getSuccessMsg())
		.match(/^Chào mừng.+quay trở lại!$/);
});
