...
test('GET / get Google product feeds', async t => {
    ...
    googleFeeds.forEach(feed => {
        ...
        t.true(feed["Page URL"].includes('https://www.leflair.vn/vn/products/'))
    })
...
test('GET / get Google Merchant product feeds', async t => {
    ...
    t.deepEqual(googleMerchantFeeds.feed._attributes.xmlns, 'http://www.w3.org/2005/Atom')
    t.deepEqual(googleMerchantFeeds.feed._attributes["xmlns:g"], 'http://base.google.com/ns/1.0')
    ...
    googleMerchantFeeds.feed.entry.forEach(entry => {
        ...
        if (entry["g:link"]._text.match(/(\?|&)color/)) {
            t.true(entry["g:link"]._text.includes(entry["g:color"]._text.normalize()))
        }
        if (entry["g:link"]._text.match(/(\?|&)size/)) {
            t.true(entry["g:link"]._text.includes(entry["g:size"]._text.normalize()))
        }
        ...
        // category must comply with https://www.google.com/basepages/producttype/taxonomy.en-US.txt
        t.true(googleCategories.includes(entry["g:google_product_category"]._text))
        ...
        if (entry["g:age_group"]._text.length > 0) {
            t.regex(entry["g:age_group"]._text, /newborn|infant|toddler|kids|adult/)
        }
...
test('GET / get Insider product feeds', async t => {
    ...
    t.deepEqual(insiderFeeds.products._attributes.xmlns, 'http://www.w3.org/2005/Atom')
