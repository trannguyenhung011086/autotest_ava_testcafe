...
test.serial.skip('POST / not split HK order when total < 1,000,000', async t => {
...
test.serial('POST / split SG and VN order - voucher (amount)', async t => {
    ...
    t.deepEqual(orders[0].paymentSummary.voucherAmount +
        orders[1].paymentSummary.voucherAmount, voucher.amount)
})
...
test.serial('POST / split SG order when total >= 1,000,000 - voucher (percentage + max discount)', async t => {
    ...
    t.true(orders[0].paymentSummary.voucherAmount + orders[1].paymentSummary.voucherAmount +
        orders[2].paymentSummary.voucherAmount <= voucher.maximumDiscountAmount)
})