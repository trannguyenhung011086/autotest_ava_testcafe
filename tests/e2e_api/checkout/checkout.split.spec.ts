...
test.serial("POST / split SG and VN order", async t => {
    if (process.env.NODE_ENV == "prod") {
        ...
        const itemSG = await requestProduct.getProductWithCountry(
            "SG",
            0,
            2000000
        );
        const itemVN = await requestProduct.getProductWithCountry(
            "VN",
            0,
            2000000
        );
...
test.serial.skip("POST / split HK and VN order", async t => {
    ...
    if (process.env.NODE_ENV == "prod") {
        ...
        const itemHK = await requestProduct.getProductWithCountry(
            "HK",
            0,
            2000000
        );
...
test.serial("POST / split multiple SG and VN order", async t => {
    if (process.env.NODE_ENV == "prod") {
        ...
        const itemVN1 = await requestProduct.getProductWithCountry(
            "VN",
            0,
            800000
        );
