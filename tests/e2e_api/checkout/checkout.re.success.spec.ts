...
test.serial("POST / recheckout with COD", async t => {
	let reCheckout = await request.checkoutCod(
		checkoutInput,
		t.context["cookie"]
	);
...
test.serial(
	...
	async t => {
		if (process.env.NODE_ENV == "prod") {
			...
			let reCheckout = await request.checkoutPayDollar(
				checkoutInput,
				t.context["cookie"]
			);
			t.truthy(reCheckout.orderId);
			...
			let result = await request.postFormUrlPlain(
				config.payDollarApi,
				payDollarCreditCard,
				t.context["cookie"],
...
test.serial("POST / recheckout with saved CC (skip-prod)", async t => {
	if (process.env.NODE_ENV == "prod") {
		...
		let matchedCard = await requestCreditcard.getCard(
			"PayDollar",
			t.context["cookie"]
		);
